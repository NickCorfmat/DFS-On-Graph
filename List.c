...
bool equals(List A, List B) {
	if (A == NULL || B == NULL) {
		fprintf(stderr, "Error: calling equals() on NULL List object(s).\n");
		exit(EXIT_FAILURE);
	}
...
void printList(FILE* out, List L) {
	...
	while (N != NULL) {
		fprintf(out, "%d%s", N->data, N==L->back?"":" ");
		N = N->next;
	}
...
List copyList(List L) {
	if (L == NULL) {
                fprintf(stderr, "Error: calling copyList() on NULL List object.\n");
                exit(EXIT_FAILURE);
        }
	...
	int len = L->length;
	int originalIndex = L->index;
	Node originalCursor = L->cursor;

	while (len > 0) {
		append(outList, get(L));
		moveNext(L);
		len -= 1;
	}
